...
/**
 * Purpose: 
 * - This class represents a watering machine in the greenhouse simulation. When triggered, it toggles the state of the watering machine between on and off, and outputs a message to the console indicating the current state of the machine.
 * 
 * Code behaviours: 
 * - The Water class extends the Event class and overrides the action method to toggle the state of the watering machine and print a message indicating the current state to the console.
 ...
 * 
 * Particulars: 
 * - Water does not manage its own timing or scheduling; it relies on the Event superclass for that functionality.
 * - Water is typically created by the Greenhouse class based on entries in the plan file, which specifies when the watering machine should be toggled.
 * 
 ...
 * Classes:
 * - Water:
 *   - A concrete subclass of Event that represents a watering event. Its primary purpose is to toggle the state of the watering machine between on and off when it is triggered.
 *  
 * Instance Variables: 
 *  - on (private boolean):
 *         - A flag indicating whether the watering machine is currently on or off. It is toggled each time the action method is called.
 * 
 * - All other instance variables are inherited from the Event superclass.
 * - InitialDelay (protected long):
 *         - The initial delay before the event is triggered, in milliseconds. Is used to determine when the watering machine should be toggled after the simulation starts.
 * - RecurringDelay (protected long):
 *         - The delay between recurring triggers of the event, in milliseconds. Is used to determine how often the watering machine should be toggled after the initial trigger.
...
public class Water extends Event {
    ...
    public void action() {
        on = !on;
        System.out.println("Watering machine " + (on ? "ON" : "OFF"));
    }
}
